...
void updateLEDs() {
  ...
  for (uint8_t charNo = 0; charNo < sizeof(buffchr)-1; charNo++) {    // cycle through characters in buffchar
    ...
    {
      ledNo = char_mapping_LED[charNo] + (segNo * LEDS_IN_SEGMENT);   // get start LED number for segment
  ...
  switch (schedCount) {
    case 0:
      if(updateTemperature()){
        updateLEDs();
        pulseColour = BLANK_COLOUR;     // kill the clock pulse if we are showing temperature
      } 
      break;
    case 10:
      if(updateTime()) {
